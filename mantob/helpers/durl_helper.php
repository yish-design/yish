...
function dr_search_url($params = NULL, $name = NULL, $value = NULL, $urlrule = NULL, $moddir = NULL) {
    ...
    if (SITE_MOBILE === TRUE) {
        // 移动端
        return SITE_URL.$dir.'/index.php?c=search&'.http_build_query($params);
    } else {
        // PC端
        $ci	= &get_instance();
        $mod = $ci->get_cache('module-'.SITE_ID.'-'.$dir);
...
function dr_catpos($catid, $symbol = ' > ', $url = TRUE) {
	...
	foreach ($array as $id) {
		if ($id && $cat[$id]) {
            $murl = SITE_MOBILE === TRUE ? SITE_URL.APP_DIR.'/index.php?c=category&id='.$id : $cat[$id]['url'];
			$name[] = $url ? "<a href=\"{$murl}\">{$cat[$id]['name']}</a>" : $cat[$id]['name'];
		}
	}

    $murl = SITE_MOBILE === TRUE ? SITE_URL.APP_DIR.'/index.php?c=category&id='.$catid : $cat[$catid]['url'];
	$name[] = $url ? "<a href=\"{$murl}\">{$cat[$catid]['name']}</a>" : $cat[$catid]['name'];
...
function dr_page_catpos($id, $symbol = ' > ') {
	...
	$page = APP_DIR ? $page[APP_DIR] : $page['index'];
	if (!isset($page[$id])) {
        return NULL;
    }
	...
	foreach ($array as $i) {
		if ($i && $page[$i]) {
            $murl = SITE_MOBILE === TRUE ? SITE_URL.(APP_DIR ? APP_DIR.'/' : '').'index.php?c=page&id='.$i : $page[$i]['url'];
			$name[] = "<a href=\"{$murl}\">{$page[$i]['name']}</a>";
		}
	}

    $murl = SITE_MOBILE === TRUE ? SITE_URL.(APP_DIR ? APP_DIR.'/' : '').'index.php?c=page&id='.$id : $page[$id]['url'];
    $name[] = "<a href=\"{$murl}\">{$page[$id]['name']}</a>";
...
function dr_module_seo($mod) {
	...
	if (version_compare(PHP_VERSION, '5.5.0') >= 0) {
        ...
        $seo['meta_title'] = preg_replace_callback('#{([a-z_0-9]+)}#U', array($rep, 'php55_replace_data'), $meta_title);
        $seo['meta_title'] = preg_replace_callback('#{([A-Z_]+)}#U', array($rep, 'php55_replace_var'), $seo['meta_title']);
        $seo['meta_keywords'] = preg_replace_callback('#{([a-z_0-9]+)}#U', array($rep, 'php55_replace_data'), $mod['setting']['seo']['module_keywords']);
        $seo['meta_keywords'] = preg_replace_callback('#{([A-Z_]+)}#U', array($rep, 'php55_replace_var'), $seo['meta_keywords']);
        $seo['meta_description'] = preg_replace_callback('#{([a-z_0-9]+)}#U', array($rep, 'php55_replace_data'), $mod['setting']['seo']['module_description']);
        $seo['meta_description'] = preg_replace_callback('#{([A-Z_]+)}#U', array($rep, 'php55_replace_var'), $seo['meta_description']);
        unset($rep);
	} else {
...
function dr_url_encode($value) {
	if (0) {
		...
		return $ci->encrypt->encode($value, 'mantob v2');
	}
	return $value;
}
...
function dr_url_decode($value) {
	if (0) {
		...
		$ci->encrypt->set_cipher(MCRYPT_BLOWFISH);
		$ci->encrypt->set_mode(MCRYPT_MODE_CFB);
		return $ci->encrypt->decode(str_replace(' ', '+', $value), 'mantob v2');
	}
	return $value;
}
